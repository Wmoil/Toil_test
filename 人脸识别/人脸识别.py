...
# 待识别的人脸图片文件夹
FaceFigPath = r'D:\Object_Detection\mtcnn_in'
# 输出人脸图片文件夹
OutFigPath = r'D:\Object_Detection\mtcnn_out'
# 数据库人脸图片编码json字符串的路经
...
# 图片数据标注函数
def plot_tip(figuredata, boxdata, txt, boxcolor=(0, 97, 255), boxw=2, txtcolor1=(255,0,0),txtcolor2=(0,0,255)):
    # 标注人脸框
    ...
    if txt == '无法识别':
        txt_data = cv2AddChineseText(box_data, txt, (x, y - 30), textColor=txtcolor1, textSize=30)
    else:
        txt_data = cv2AddChineseText(box_data, txt, (x, y - 30), textColor=txtcolor2, textSize=30)
    ...
    for per_fig in os.listdir(inp):
        ...
        if result_detection:
            ...
            for face in result_detection:
                ...
                tform.estimate(np.array(src_data), dstdata)
                M = tform.params
                # 应用仿射矩阵进行人脸对齐
        ...
        dic_trans = {sn: '无法识别', 'wanghaojie': '王豪杰', 'yangqun': '杨群', 'huangbo': '黄渤', 'lijiaxin': '李嘉欣', 'wangxun': '王迅'}
        ...
        # 保存为图片
        cv2.imwrite(r'%s/%s.png' % (oup, per_fig.split('.')[0]), image_data, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
